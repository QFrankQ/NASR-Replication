#=================Changes Made===============
# Added several other metrics in intuition eval
# Added pos_weights argument
#=================Changes Made===============
import os
import json
import argparse
...
import statistics
from models.transformer_sudoku import get_model, get_no_argmax_model
...
def final_output(model,ground_truth_sol,solution_boards,masking_boards,args):
    ...
    # add argument with if statement
    # Something like this: masking_prob = (masking_prob > 0.70) * masking_prob
...
def intuition_eval(input_boards,y_pred,y_pred_b,mask_b,y_targ,args):
    ...
    print("\n---\n")
    print(f"Num errors of the solverNN out of 81 cells(avg): {statistics.mean(n_error_ns)}%)") #added
    print(f"Percentage of solverNN errors in the input out of 81 cells(avg): {statistics.mean(perc_error_ns_i)}%)") #added
    print(f"Percentage of solverNN errors in the solutions out of 81 cells(avg): {statistics.mean(perc_error_ns_s)}%)")#added
    print(f"Percentage of solverNN errors in the input masked out of all incorrect input (avg): {statistics.mean(perc_error_ns_masked_i)}%)")   
    print(f"Percentage of solverNN errors in the solutions masked out of all incorrect solutions(avg): {statistics.mean(perc_error_ns_masked_s)}%)")
    print(f"Number of SolverNN correct solutions being preserved by Mask-Predictor(avg): {statistics.mean(perc_correct_ns_preserved_s)}%")
    print(f"Number of cells masked by Mask-Predictor(avg): {statistics.mean(perc_cells_masked)}%")
    print(f"Percentage of solverNN errors in the input corrected out of all incorrect input (avg): {statistics.mean(perc_error_ns_corrected_i)}%)")   
    print(f"Percentage of solverNN errors in the solutions corrected out of all incorrect solutions(avg): {statistics.mean(perc_error_ns_corrected_s)}%)")
...
def main():
    ...
    # Model
    if args.modified_module == True:
        model = get_no_argmax_model(block_len=args.block_len)
    else:
