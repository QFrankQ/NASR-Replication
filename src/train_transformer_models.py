...
# added the pos_weight argument to the model path in main loop
#=============== modifications to the file==========
...
def main():
    ...
    best_loss = 100.
    time_begin = time()
    with open(f"{args.module}/log_{args.pos_weights}.txt", 'w'): pass
    for epoch in range(args.epochs):
        ...
        if loss < best_loss:
            best_loss = loss
            torch.save(model.state_dict(), f'{args.module}/checkpoint_best_{args.pos_weights}.pth')

        stats = {'epoch': epoch, 'lr': lr,
                 'train_loss': train_loss, 'val_loss': loss, 'best_loss': best_loss}
        with open(f"{args.module}/log_{args.pos_weights}.txt", "a") as f:
    ...
    torch.save(model.state_dict(), f'{args.module}/checkpoint_last_{args.pos_weights}.pth')
    print_loss_graph_from_file(f"{args.module}/log_{args.pos_weights}.txt",f"{args.module}/loss_{args.pos_weights}.png")
