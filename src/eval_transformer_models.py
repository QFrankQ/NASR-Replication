#================Changes Made==============
# Refined the print statements for better explanation
#================Changes Made==============
import os
import json
import argparse
...
def eval_solver(y_pred,y_targ,input_b,dataset_name = None):
    ...
    print(f'Preservation of hint cells: {np.mean(stats_input):.4f}')
    print(f'Correctness of solutions out of 81 cells (avg): {np.mean(stats_sol):.4f}')
    print(f'Completely correct solution boards (avg): {totally_correct}/{len(pred_boards)} ({totally_correct*100/len(pred_boards):.2f}%)')
    for k in range(len(pred_boards)):
...
def main():
    ...
    # ----------------------------------
    # ckpt_path = f'outputs/{args.module}/{args.data}/checkpoint_best_{args.pos_weights}.pth'
