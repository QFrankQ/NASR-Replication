...
def validate(val_loader, model, args, epoch=None, time_begin=None):
    ...
    labels = []
    inputs = [] #images
    solutions = [] #Neural Solver Solutions
    mask = []
    ...
    with torch.no_grad():
        for i, (images, target) in enumerate(val_loader):
            ...
            print(f"Neural Solver solution shapes{solution_boards.shape}")
            print(f"Mask Predictor prediction shapes{solution_boards.shape}")
            return 
            ...
            solutions.append(solution_boards) #Neural Solver solution boards
...
def intuition_eval(input_boards,y_pred,y_pred_b,mask_b,y_targ,args):
    ...
    for i in tqdm(range(len(y_pred))):
        ...
        if ncns_s != 0: #added
            n_correct_ns_preserved_s.append(ncns_p_s/ncns_s)
        if nep !=0: #added
        ...
        if nens_i!=0:
            ...
            n_error_ns_i.append(nens_i/81)#added
        if nens_s!=0:
            n_error_ns_masked_s.append(nens_m_s/nens_s)
            n_error_ns_s.append(nens_s/81)#added
    ...
    print(f"Num errors of the solverNN in the input out of 81 cells(avg): {statistics.mean(n_error_ns_i)}%)") #added
    print(f"Num errors of the solverNN in the solutions out of 81 cells(avg): {statistics.mean(n_error_ns_s)}%)")#added
    print(f"Num errors of the solverNN in the input masked out of all incorrect input (avg): {statistics.mean(n_error_ns_masked_i)}%)")   
    print(f"Num errors of the solverNN in the solutions masked(avg) out of all incorrect solutions: {statistics.mean(n_error_ns_masked_s)}%)")
    print(f"Number of SolverNN correct solutions being preserved by Mask-Predictor(avg): {statistics.mean(n_correct_ns_preserved_s)}%")   
